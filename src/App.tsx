...
const tagBadgeVariant: Record<string, BadgeVariant> = {
  nice: 'success',
  developer: 'info',
  kawaii: 'warning',
  cool: 'success',
...
const tableBasicData: DemoPerson[] = [
...
const tableSortableData: DemoPerson[] = [...tableBasicData, { id: 4, name: 'Jim Red', age: 32, address: 'London No. 2 Lake Park', tags: [] }];

const tableSelectionData: DemoPerson[] = [
  ...tableBasicData,
  { id: 4, name: 'Disabled User', age: 99, address: 'Sydney No. 1 Lake Park', tags: [] },
];

function nameCell(name: string) {
  return (
    <Typography as="span" variant="body" color="var(--secondary-600)">
      {name}
    </Typography>
  );
}

const tableBasicColumns: TableColumn<DemoPerson>[] = [
  { key: 'name', title: 'Name', render: (_v, record) => nameCell(record.name) },
  ...
  { key: 'address', title: 'Address', dataIndex: 'address' },
  {
    ...
    title: 'Tags',
    render: (_v, record) => (
      <div style={{ display: 'flex', gap: 'var(--spacing-space-4)', flexWrap: 'wrap' }}>
        {record.tags.map((tag) => (
          <Badge key={tag} variant={tagBadgeVariant[tag] ?? 'neutral'}>
            {tag.toUpperCase()}
          </Badge>
        ))}
      </div>
  ...
  },
  {
    ...
    render: (_v, record) => (
      <div style={{ display: 'flex', gap: 'var(--spacing-space-12)' }}>
        <Button variant="tertiary">Invite {record.name}</Button>
...
const tableSortableColumns: TableColumn<DemoPerson>[] = [
  { key: 'name', title: 'Name', dataIndex: 'name', sortable: true, render: (_v, record) => nameCell(record.name) },
  { key: 'age', title: 'Age', dataIndex: 'age', sortable: true },
  { key: 'address', title: 'Address', dataIndex: 'address' },
];

const tableSelectionColumns: TableColumn<DemoPerson>[] = [
  { key: 'name', title: 'Name', render: (_v, record) => nameCell(record.name) },
  { key: 'age', title: 'Age', dataIndex: 'age' },
  { key: 'address', title: 'Address', dataIndex: 'address' },
];

function generateEdwardKings(count: number) {
  return Array.from({ length: count }, (_, i) => ({
    id: i,
    name: `Edward King ${i}`,
    age: 32,
...
const tableReloadColumns: TableColumn<ReturnType<typeof generateEdwardKings>[number]>[] = [
  { key: 'name', title: 'Name', render: (_v, record) => nameCell(record.name) },
  { key: 'age', title: 'Age', dataIndex: 'age' },
  { key: 'address', title: 'Address', dataIndex: 'address' },
];
...
const statusBadgeVariant: Record<DemoUser['status'], BadgeVariant> = {
  active: 'success',
  pending: 'warning',
  banned: 'error',
};

const tableRichData: DemoUser[] = [
  {
    id: 1,
    name: 'Aarav Sharma',
    username: '@aarav',
    ...
    rating: 4,
    bio: 'Frontend engineer focused on design systems.',
  },
  {
    id: 2,
    name: 'Meera Nair',
    username: '@meera',
    createdAt: '2024-02-03 14:05',
    status: 'pending',
    tags: ['trial'],
    rating: 3,
    bio: 'Product designer exploring healthcare UX.',
  },
  {
    id: 3,
    name: 'Rohan Verma',
    ...
    tags: ['flagged'],
    rating: 2,
    bio: 'Reported for a policy violation.',
  },
  {
    id: 4,
    name: 'Divya Iyer',
    username: '@divya',
    createdAt: '2024-03-01 08:15',
    status: 'active',
    tags: ['premium'],
    rating: 5,
    bio: 'Backend lead, API architecture.',
  },
  {
    id: 5,
    name: 'Kabir Khan',
    username: '@kabirk',
    createdAt: '2024-03-15 17:50',
    status: 'active',
    tags: ['verified', 'beta'],
    rating: 4,
  ...
  },
  {
    ...
    name: 'Sara Fernandes',
    username: '@saraf',
    createdAt: '2024-03-22 12:00',
    status: 'pending',
    tags: ['trial'],
    rating: 3,
    bio: 'Growth marketer, onboarding flows.',
  },
];

const tableRichColumns: TableColumn<DemoUser>[] = [
  {
    key: 'sno',
    title: 'S.No',
    align: 'center',
  ...
  },
  {
    ...
    title: 'User',
    render: (_v, record) => (
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-space-8)' }}>
        <div
          style={{
            width: 32,
            height: 32,
  ...
  { key: 'createdAt', title: 'Date & Time', dataIndex: 'createdAt', sortable: true },
  {
    key: 'status',
    title: 'Status',
    render: (_v, record) => <Badge variant={statusBadgeVariant[record.status]}>{record.status.toUpperCase()}</Badge>,
  },
  {
    key: 'tags',
    title: 'Tags',
  ...
  },
  {
    key: 'rating',
    title: 'Rating',
    render: (_v, record) => <Rate value={record.rating} disabled size={16} />,
  },
  {
    key: 'bio',
    title: 'Bio',
    width: 220,
    render: (_v, record) => (
      <Typography as="span" variant="caption" color="var(--neutral-600)">
        {record.bio}
      </Typography>
    ),
  },
  {
    ...
    align: 'right',
    render: () => (
      <div style={{ display: 'flex', gap: 'var(--spacing-space-8)', justifyContent: 'flex-end' }}>
...
const tableMinimalColumns: TableColumn<DemoPerson>[] = [
  { key: 'name', title: 'Name', render: (_v, record) => nameCell(record.name) },
  { key: 'address', title: 'Address', dataIndex: 'address' },
];
...
function TableSelectionDemo() {
  ...
  return (
    <Table columns={tableSelectionColumns} data={tableSelectionData} rowKey="id" rowSelection={rowSelection} />
  );
}

function TableReloadDemo() {
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState(() => generateEdwardKings(9));

  const handleReload = () => {
    setLoading(true);
    setTimeout(() => {
      setData(generateEdwardKings(9));
      setLoading(false);
    }, 800);
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-12)' }}>
      ...
      <Table columns={tableReloadColumns} data={data} rowKey="id" loading={loading} pagination={false} />
    </div>
  );
}

function TableKitchenSinkDemo() {
  ...
  const filtered = tableRichData.filter((user) => {
    ...
    const matchesTag = tagFilter.length === 0 || user.tags.some((tag) => tagFilter.includes(tag));
    const matchesRating = ratingFilter.length === 0 || String(user.rating) === ratingFilter[0];
    const matchesMonth = monthFilter.length === 0 || user.createdAt.startsWith(monthFilter[0]);
    return matchesSearch && matchesStatus && matchesTag && matchesRating && matchesMonth;
  });

  const rowSelection: TableRowSelection<DemoUser> = {
    selectedRowKeys,
    onChange: setSelectedRowKeys,
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-16)' }}>
      <div style={{ display: 'flex', gap: 'var(--spacing-space-12)', flexWrap: 'wrap' }}>
        <div style={{ width: 240 }}>
          <Search placeholder="Search name or username" value={search} onChange={setSearch} />
        </div>
        <div style={{ width: 180 }}>
          <Dropdown
            mode="normal"
      ...
      <Table
        columns={tableRichColumns}
        data={filtered}
        rowKey="id"
        rowSelection={rowSelection}
        pagination={{ pageSize: 5 }}
        headerBackground="var(--primary-50)"
      />
    </div>
  );
}

const tourBasicSteps: TourStep[] = [
...
const tourCustomActionSteps: TourStep[] = [
  {
    title: 'Custom Actions',
    description: 'This step replaces the default Previous/Next with fully custom footer buttons.',
    actions: ({ onNext, onClose }) => (
      <div style={{ display: 'flex', gap: 'var(--spacing-space-8)', justifyContent: 'flex-end', width: '100%' }}>
...
function TourAutoStartDemo() {
  const [open, setOpen] = useState(() => !window.localStorage.getItem('milton-tour-demo-seen'));

  const handleClose = () => {
    setOpen(false);
    window.localStorage.setItem('milton-tour-demo-seen', '1');
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-12)' }}>
      <div style={row}>
        <Button
          variant="secondary"
          onClick={() => {
            window.localStorage.removeItem('milton-tour-demo-seen');
            setOpen(true);
          }}
        >
          Reset "first visit" flag &amp; show again
        </Button>
      </div>
      <Tour steps={tourBasicSteps} open={open} onClose={handleClose} onFinish={handleClose} />
    </div>
  );
}

function TourTargetedDemo() {
  ...
  const steps: TourStep[] = [
    { title: 'Upload File', description: 'Put your files here.', target: uploadRef, placement: 'bottom' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-16)' }}>
      ...
      <div style={row}>
        ...
        <IconButton ref={settingsRef} icon={Settings01Icon} variant="secondary" aria-label="Settings" />
      </div>
...
function TourVariantDemo() {
  const [variant, setVariant] = useState<TourVariantKey | null>(null);
  ...
  const configs: Record<
    TourVariantKey,
    {
      ...
      closable?: boolean;
      showSkip?: boolean;
      mask?: 'none' | 'blur' | 'dimmed';
      type?: 'default' | 'primary';
      indicatorRender?: (current: number, total: number) => ReactNode;
    }
  > = {
    default: { steps: tourBasicSteps },
    noSkip: { steps: tourBasicSteps, showSkip: false },
    noClose: { steps: tourBasicSteps, closable: false },
    forced: { steps: tourBasicSteps, closable: false, showSkip: false },
    customActions: { steps: tourCustomActionSteps },
    maskBlur: { steps: tourBasicSteps, mask: 'blur' },
    maskNone: { steps: tourBasicSteps, mask: 'none' },
    primaryType: { steps: tourBasicSteps, type: 'primary' },
    customIndicator: {
      steps: tourBasicSteps,
      indicatorRender: (current, total) => <span>{`${current + 1} / ${total}`}</span>,
    },
  };

  const buttons: { key: TourVariantKey; label: string }[] = [
    { key: 'default', label: 'With Skip + Close' },
    { key: 'noSkip', label: 'Without Skip' },
    { key: 'noClose', label: 'Without Close' },
    { key: 'forced', label: 'Forced (no skip/close)' },
    { key: 'customActions', label: 'Custom Actions' },
    { key: 'maskBlur', label: 'Blur Mask' },
    { key: 'maskNone', label: 'No Mask' },
    { key: 'primaryType', label: 'Primary Type' },
    { key: 'customIndicator', label: 'Custom Indicator' },
  ];

  const active = variant ? configs[variant] : null;

  return (
    <>
      <div style={row}>
        {buttons.map((b) => (
          <Button key={b.key} variant="primary" onClick={() => setVariant(b.key)}>
            {b.label}
          </Button>
        ))}
      </div>
      {active && (
        <Tour
          ...
          showSkip={active.showSkip}
          mask={active.mask}
          type={active.type}
          indicatorRender={active.indicatorRender}
          onClose={() => setVariant(null)}
          onFinish={() => setVariant(null)}
        />
      )}
    </>
  );
...
export default function App() {
  ...
  const [timelineReversed, setTimelineReversed] = useState(false);
      ...
      >
        ...
        <section style={sectionStyle}>
          ...
          <div style={inputGrid}>
            ...
            <div style={inputCell}>
              ...
              <PasswordInput label="Label" required showRequirements defaultValue="Gtu6k_kdfj" />
            </div>
          </div>
        </section>
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ ...row, marginTop: 'var(--spacing-space-12)' }}>
            <Badge variant="neutral" icon={LocationIcon}>
              Neutral
            </Badge>
            <Badge variant="info" icon={LocationIcon}>
              Info
            </Badge>
            <Badge variant="success" icon={LocationIcon}>
              Success
            </Badge>
            <Badge variant="warning" icon={LocationIcon}>
              Warning
            </Badge>
            <Badge variant="error" icon={LocationIcon}>
              Error
            </Badge>
          </div>
        </section>
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ ...row, marginTop: 'var(--spacing-space-12)' }}>
            ...
            {chipSelected ? (
              <Chip selected onClick={() => {}} onRemove={() => setChipSelected(false)}>
                Selected
              </Chip>
            ) : (
        ...
        <section style={sectionStyle}>
          ...
          >
            ...
            <Divider variant="dash" textAlign="center">
              Text
            </Divider>
          </div>
        </section>
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--spacing-space-24)' }}>
            ...
            >
              ...
              <Alert type="error" level="secondary">
            ...
            >
              ...
              <Alert type="error" level="secondary" showIcon={false}>
                Error message will appear here
              </Alert>
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ padding: 'var(--spacing-space-24)' }}>
            ...
            <div
              style={{
                ...row,
                gap: 'var(--spacing-space-40)',
                marginBottom: 'var(--spacing-space-40)',
              }}
            >
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-24)' }}>
            ...
            <div style={subCell}>
              ...
              >
                <div
                  style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-space-12)' }}
                >
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-24)' }}>
            <div style={subCell}>
              ...
              <div
                style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-8)' }}
              >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-24)' }}>
            ...
            <div style={subCell}>
              <Typography variant="h3">
                Horizontal — 3 Levels (Professionals → Doctors → Specialists)
              </Typography>
              <div
                style={{
                  border: '1px solid var(--neutral-100)',
            ...
            <div style={subCell}>
              <Typography variant="h3">
                Vertical — Default (accordion submenu, nested levels)
              </Typography>
              <div
                style={{
                  width: 280,
              ...
              >
                <Navigation
                  items={navVerticalItems}
                  mode="vertical"
                  defaultOpenKeys={['nav-two']}
                  accordion
                />
              </div>
            </div>
            ...
            <div style={subCell}>
              <Typography variant="h3">
                Vertical — Collapsible (icon-only rail, hover tooltip)
              </Typography>
              <div
                style={{
                  width: 280,
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-24)' }}>
            <div style={subCell}>
              ...
              <div
                style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-16)' }}
              >
                <div>
            ...
            <div style={subCell}>
              ...
              <div
                style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-12)' }}
              >
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-24)' }}>
            ...
            <div style={subCell}>
              ...
              <div
                style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-24)' }}
              >
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-24)' }}>
            ...
            <div style={subCell}>
              ...
              <Header
                eyebrowTags={[
                  { key: 'verified', label: 'Certified & Verified', tone: 'secondary' },
                ]}
                title={
                  <>
                    Discover Trusted{' '}
                    <span style={{ color: 'var(--primary-600)' }}>Health Services </span>Near You
                  </>
                }
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-24)' }}>
            ...
            <div style={subCell}>
              <Typography variant="h3">
                3. Dot + Left &amp; Right Icon — Top / Bottom / Start / End
              </Typography>
              <div
                style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-16)' }}
              >
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-24)' }}>
            <div style={subCell}>
              ...
              <div style={row}>
                <NotificationDemo label="topLeft" placement="topLeft" />
                <NotificationDemo label="topRight" placement="topRight" />
                <NotificationDemo label="bottomLeft" placement="bottomLeft" />
                <NotificationDemo label="bottomRight" placement="bottomRight" />
                <NotificationDemo label="top" placement="top" />
                <NotificationDemo label="bottom" placement="bottom" />
              </div>
            </div>
            ...
            <div style={subCell}>
              ...
              <div style={row}>
                <NotificationDemo label="Default" />
                <NotificationDemo label="Success" type="success" />
                <NotificationDemo label="Info" type="info" />
                <NotificationDemo label="Warning" type="warning" />
                <NotificationDemo label="Error" type="error" />
              </div>
            </div>
            ...
            <div style={subCell}>
              ...
              <div style={row}>
                <NotificationDemo label="Persistent (duration 0)" duration={0} />
                <NotificationDemo label="Quick (1.5s)" duration={1500} />
              </div>
            </div>
          </div>
        </section>
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-24)' }}>
            <div style={subCell}>
              <Typography variant="h3">Basic (left placement)</Typography>
              <div style={{ maxWidth: 480 }}>
            ...
            <div style={subCell}>
              <Typography variant="h3">Center placement (alternate)</Typography>
            ...
            <div style={subCell}>
              <Typography variant="h3">Pending + Reverse</Typography>
              <div style={row}>
                <Button variant="primary" onClick={() => setTimelineReversed((r) => !r)}>
                  Toggle Reverse
                </Button>
              </div>
              <div style={{ maxWidth: 480 }}>
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-24)' }}>
            <div style={subCell}>
              <Typography variant="h3">Basic</Typography>
              <Table columns={tableBasicColumns} data={tableBasicData} rowKey="id" pagination={false} />
            </div>

            <div style={subCell}>
              <Typography variant="h3">Row Selection (with disabled row)</Typography>
              <TableSelectionDemo />
            </div>

            <div style={subCell}>
              <Typography variant="h3">Sortable Columns</Typography>
              <Table columns={tableSortableColumns} data={tableSortableData} rowKey="id" pagination={false} />
            </div>

            <div style={subCell}>
              <Typography variant="h3">Loading + Reload</Typography>
              <TableReloadDemo />
            </div>

            <div style={subCell}>
              <Typography variant="h3">Minimal (two columns)</Typography>
              <Table columns={tableMinimalColumns} data={tableBasicData} rowKey="id" pagination={false} />
            </div>

            <div style={subCell}>
              <Typography variant="h3">
                Full Table (filter toolbar, avatar, badges, tags, rating, actions, sortable, paginated)
              </Typography>
              <TableKitchenSinkDemo />
            </div>
          </div>
        </section>
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-24)' }}>
            <div style={subCell}>
              <Typography variant="h3">No Internet</Typography>
            ...
            <div style={subCell}>
              <Typography variant="h3">500 Server Error</Typography>
              <div style={{ border: '1px solid var(--neutral-300)', borderRadius: 'var(--radius-border-radius-8)' }}>
                <EmptyState preset="serverError" />
              </div>
            </div>
            ...
            <div style={subCell}>
              <Typography variant="h3">Custom Override (e.g. inside a Table)</Typography>
              <div style={{ border: '1px solid var(--neutral-300)', borderRadius: 'var(--radius-border-radius-8)' }}>
                <EmptyState
                  preset="noData"
        ...
        <section style={sectionStyle}>
          <Typography variant="h2">Tour</Typography>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-space-24)' }}>
            <div style={subCell}>
              <Typography variant="h3">Auto-start on first visit (localStorage flag)</Typography>
              <TourAutoStartDemo />
            </div>

            <div style={subCell}>
              <Typography variant="h3">Targeted Steps (real elements, ref-based)</Typography>
              <TourTargetedDemo />
            </div>

            <div style={subCell}>
              <Typography variant="h3">
                Skip / Close / Mask / Actions / Indicator / Type Variants
              </Typography>
              <TourVariantDemo />
            </div>
          </div>
